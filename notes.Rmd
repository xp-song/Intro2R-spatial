...
Import all 7 bands from the satellite data as a `RasterStack` object named `landsat`:  
...
Set 0.4 as the threshold; reclassify values below this threshold to `NA`: 

```{r reclassify based on threshold}
landsatGreen <- reclassify(landsatNDVI, c(-1, 0.4, NA)) #-1 to 0.4 becomes NA
```

Plot values of NDVI larger than 0.4
